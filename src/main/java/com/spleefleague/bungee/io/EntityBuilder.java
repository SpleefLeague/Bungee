...
import com.spleefleague.bungee.util.MapUtil;
...
public class EntityBuilder {
    ...
    public static Document serialize(DBSaveable object) {
        try {
            HashMap<String, IOClass.Output> outputs = getOutputs(object.getClass());
    ...
    public static <T> T deserialize(Document dbo, Class<T> c) {
        try {
            ...
            Map<String, IOClass.Input> inputs = getInputs(c);
            for (String name : inputs.keySet()) {
                Object o = dbo.get(name);
                if (o != null) {
                    IOClass.Input i = inputs.get(name);
                    i.set(t, o);
                }
            }
            ((DBLoadable) t).done();
    ...
    private static Map<String, IOClass.Input> getInputs(Class c) {
        Map<String, IOClass.Input> inputs = new HashMap<>();
        Class current = c;
        while (current != null && current.isAssignableFrom(c)) {
            for (Method m : current.getDeclaredMethods()) {
                DBLoad dbload = m.getAnnotation(DBLoad.class);
                if (dbload != null) {
                    inputs.put(dbload.fieldName(), new IOClass.Input(dbload.priority(), m));
                }
            ...
            for (Field f : current.getDeclaredFields()) {
                ...
                if (dbload != null) {
                    inputs.put(dbload.fieldName(), new IOClass.Input(dbload.priority(), f));
                }
            }
    ...
    private static HashMap<String, IOClass.Output> getOutputs(Class c) {
        HashMap<String, IOClass.Output> outputs = new HashMap<>();
        Class current = c;
        while (current != null && current.isAssignableFrom(c)) {
            for (Method m : current.getDeclaredMethods()) {
                DBSave dbsave = m.getAnnotation(DBSave.class);
                if (dbsave != null) {
                    outputs.put(dbsave.fieldName(), new IOClass.Output(dbsave.priority(), m));
                }
            ...
            for (Field f : current.getDeclaredFields()) {
                ...
                if (dbsave != null) {
                    outputs.put(dbsave.fieldName(), new IOClass.Output(dbsave.priority(), f));
                }
            }
            current = current.getSuperclass();
        }
